...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticRoutes = ['', '/about', '/projects', '/blogs', '/services', '/contact', '/resume', '/sitemap', '/faq', '/technical-skills'].map((route) => ({
    url: `${APP_URL}${route}`,
    lastModified: new Date(),
    changeFrequency: (route === '' ? 'weekly' : 'monthly') as 'weekly' | 'monthly',
    priority: route === '' ? 1 : route === '/sitemap' ? 0.7 : route === '/faq' ? 0.8 : route === '/technical-skills' ? 0.8 : 0.9,
  }));
